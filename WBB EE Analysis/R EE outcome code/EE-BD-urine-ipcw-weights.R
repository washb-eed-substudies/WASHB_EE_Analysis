...
ipcw_weights <- function(outcome, Y, tr, W = NULL, id = 1:length(Y), pair = NULL, Delta = rep(1, 
    ...
    if (!is.null(W)) {
        ...
        Wscreen <- washb_prescreen(Y = tmledat$Y[tmledat$Delta == 
            1], Ws = subset(tmledat, tmledat$Delta == 1, select = names(W)), 
        ...
        if (length(Wscreen) > 0) {
            ...
            if (ncol(Wselect) <= 1 & (length(grep("SL.glmnet", 
                Q.SL.library)) + length(grep("SL.glmnet", g.SL.library)) > 
                0)) {
    ...
    tmle_fit <- tmle(Y = tmle_Y, A = tmle_A, W = Wselect, Delta = tmle_Delta, 
        id = tmle_id, family = family, Q.SL.library = Q.SL.library, 
...
for(i in 1:ncol(Y)){
  for(j in 1:3){
    ...
    temp<-ipcw_weights(outcome=colnames(Y)[i], Y=log(Y[,i]), Delta=miss[,i], tr=d$tr, W=Wlist[[i]], id=d$block, pair=NULL, family="gaussian", contrast= contrasts[[j]], Q.SL.library = c("SL.glm"), seed=12345, print=T)
    weight_list[[3*(i-1)+j]] <- temp
    }
}
 
 N<-length(weight_list[[1]]$weights)
 df <- data.frame(weights=weight_list[[1]]$weights, tr=factor(weight_list[[1]]$A), comparison=rep(paste0(weight_list[[1]]$outcome,": ", weight_list[[1]]$contrast[1], " vs. ", weight_list[[1]]$contrast[2]),N))
